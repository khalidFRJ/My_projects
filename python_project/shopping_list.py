...
def add_item (shopping_list): # for add the item 
   item = input("- Enter the item you want to add: \n")
   shopping_list.append(item)
   print(f"you add '{item}' to the list ")

def remove_item (shopping_list): # for remove the item
   item = str(input("- Enter the item you want to remove : \n"))
   if item in shopping_list:
      shopping_list.remove(item)
      print(f"you remove {item} from the list")
   else:                    
    print(f"Item '{item}' is not in the list.") # the item not in the list 
...
def view_list (shopping_list):
   ...
   if shopping_list :
      ...
      print(" - ".join(items_with_index)) # join => To merge texts with a space between them
   else :
